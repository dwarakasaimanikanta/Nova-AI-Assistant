...
def test_browser_actions() -> None:
    """Ensure BrowserTool invokes BrowserManager with correct parameters."""
    mock_manager = MagicMock()
    mock_manager.open_url.return_value = "Success: Navigated."
    mock_manager.search_google.return_value = "Success: Searched."
    ...
    mock_manager.open_url.assert_called_with("https://www.google.com")
    ...
    assert "Success" in res
    mock_manager.open_url.assert_called_with("https://www.github.com")

    # 4. open_chatgpt
    res = tool.execute(action="open_chatgpt")
    assert "Success" in res
    mock_manager.open_url.assert_called_with("https://chatgpt.com")

    # 5. open_url (with scheme)
    res = tool.execute(action="open_url", url="https://example.com/test")
    assert "Success" in res
    mock_manager.open_url.assert_called_with("https://example.com/test")

    # 6. open_url (without scheme - should prepend https://)
    res = tool.execute(action="open_url", url="example.com/abc")
    assert "Success" in res
    mock_manager.open_url.assert_called_with("https://example.com/abc")

    # 7. google_search
    res = tool.execute(action="google_search", query="Nova AI Assistant")
    assert "Success" in res
    mock_manager.search_google.assert_called_with("Nova AI Assistant")

    # 8. youtube_search
    res = tool.execute(action="youtube_search", query="lofi hip hop")
    assert "Success" in res
    mock_manager.open_url.assert_called_with("https://www.youtube.com/results?search_query=lofi+hip+hop")
